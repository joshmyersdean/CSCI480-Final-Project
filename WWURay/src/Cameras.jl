...
function PerspectiveCamera(eye::Vec3, view::Vec3, up::Vec3, focal::Real, canv_height::Int, canv_width::Int)
    # set up basis
    view = normalize(-view)
    ub = normalize(cross(up, view))
    vb = cross(view, ub)
    view *= focal
    PerspectiveCamera(eye, ub, vb, view, focal, canv_height, canv_width)
end

function pixel_to_ray(camera::PerspectiveCamera, i, j) 
     vp_height = camera.canv_height / camera.canv_width # aspect ratio                     
     u =  ((j + 0.5) / camera.canv_width - 0.5) # u weight
     v = -(((i + 0.5)/ camera.canv_height) - 0.5) * vp_height # v weight
     p = camera.eye 
     d = -camera.w_axis + u*camera.u_axis + v*camera.v_axis
     return Ray(p, d)
end
